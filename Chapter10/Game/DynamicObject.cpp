...
void DynamicObject::update(std::vector<std::vector<StaticObject>>& static_objects)
{
	...
	if (mType == Type::TYPE_1P) {
		mD = { 0,0 };
		if (Pad::isOn(Pad::Button::U)) mD.first = -1;
		else if (Pad::isOn(Pad::Button::D)) mD.first = 1;
		if (Pad::isOn(Pad::Button::L)) mD.second = -1;
		else if (Pad::isOn(Pad::Button::R)) mD.second = 1;
	}
	else if (mType == Type::TYPE_2P) {
		mD = { 0,0 };
		if (Pad::isOn(Pad::Button::U, 1)) mD.first = -1;
		else if (Pad::isOn(Pad::Button::D, 1)) mD.first = 1;
		if (Pad::isOn(Pad::Button::L, 1)) mD.second = -1;
		else if (Pad::isOn(Pad::Button::R, 1)) mD.second = 1;
	}
